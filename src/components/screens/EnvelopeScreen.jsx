...
/* ── The actual card ──────────────────────────────────────────────────────── */
function RoleCard({ role, revealed, tiltX, tiltY, isAr }) {
...
export default function EnvelopeScreen({ user, onExpire }) {
  ...
  // Auto-flip at 7s remaining — but ONLY once myRole has arrived from Firebase
  useEffect(() => {
    ...
    >
      ...
      {myRole && (
        <motion.div
          ...
          style={{ background:`radial-gradient(ellipse, ${(ROLE_CONFIG[role]??ROLE_CONFIG.citizen).glow}77 0%, transparent 70%)` }}
        />
      )}
      ...
      >
        <RoleCard role={role} revealed={revealed} tiltX={rawTiltX} tiltY={rawTiltY} isAr={isAr} />
      </div>
      ...
      >
        <p className="text-smoke-500 text-xs tracking-[0.2em] font-mono">
          {!revealed 
            ? (isAr ? 'اضغط للكشف عن دورك' : 'TAP TO REVEAL YOUR ROLE') 
            : (isAr ? 'حافظ على سرية هذا الدور' : 'KEEP THIS ROLE SECRET')}
        </p>
        {revealed && (
          <motion.div
          ...
          >
            ...
            <span className="text-smoke-400 text-[10px] uppercase tracking-widest">
              {isAr ? 'تبدأ اللعبة قريباً' : 'Game starting soon'}
            </span>
          </motion.div>
        )}
      </motion.div>
